...
def extract_frame(vid_item):
    ...
    if task == 'rgb':
        ...
        if args.input_frames:
            if args.new_short == 0:
                cmd = osp.join(
                    f"denseflow '{full_path}' -a={method} -b=20 -s=1 -o='{temp_full_path}'"  # noqa: E501
        ...
        if args.new_short == 0:
            cmd_rgb = osp.join(
                f"denseflow '{full_path}' -b=20 -s=0 -o='{temp_full_path}'"
                f' -nw={args.new_width} -nh={args.new_height} -v')
            cmd_flow = osp.join(
                f"denseflow '{full_path}' -a={method} -b=20 -s=1 -o='{temp_full_path}'"  # noqa: E501
                f' -nw={args.new_width} -nh={args.new_height} -v')
    ...
    if run_success == 0 and task == 'flow':
        video_name = osp.basename(full_path).replace(f".{args.ext}", "")
        # parrent_path = osp.dirname(temp_full_path)

        flow_x_img_path = f'{temp_full_path}/{video_name}/flow_x_%05d.{args.tmp_format}'  # noqa: E501
        flow_x_video_path = f'{out_full_path}/{video_name}_flow_x.{args.ext}'
        cmd_x = f'ffmpeg -hide_banner -loglevel error -y -i "flow_x_img_path" "{flow_x_video_path}"' # noqa: E501
        run_success_x = os.system(cmd_x)

        flow_x_img_path = f'{temp_full_path}/{video_name}/flow_y_%05d.{args.tmp_format}'  # noqa: E501
        flow_y_video_path = f'{out_full_path}/{video_name}_flow_y.{args.ext}'
        cmd_y = f'ffmpeg -hide_banner -loglevel error -y -i "flow_y_img_path" "{flow_y_video_path}"' # noqa: E501
        run_success_y = os.system(cmd_y)
        if run_success_x == 0 and run_success_y == 0:
            run_success = 0
            os.system(f'rm -r "{temp_full_path}/{video_name}/"')

            if args.concat:
                concat_path = f'{out_full_path}/{video_name}_flow.{args.ext}'
                concat_cmd = f'ffmpeg -hide_banner -loglevel error -y -i "concat:{flow_x_video_path}|{flow_y_video_path}" -c copy "{concat_path}"' # noqa: E501
                run_success = os.system(concat_cmd)
                if run_success == 0:
                    os.system(f'rm "{flow_x_video_path}" "{flow_y_video_path}"')
...
def parse_args():
    ...
    parser.add_argument(
        '--tmp-dir',
        type=str,
        default='/tmp/extract_flow',
        help='temp directory to save extracted frames')
    parser.add_argument(
        '--tmp-format',
        type=str,
        default='jpg',
        choices=['jpg'], # TODO: check if other formats are supported
        help='temporal flow images format')
    parser.add_argument(
        '--out-format',
        type=str,
        default='mp4',
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=True,
        help='concat flow x and flow y to a single video')
...
if __name__ == '__main__':
    ...
    if not osp.isdir(args.tmp_dir):
        print(f'Creating folder: {args.tmp_dir}')
    ...
    print(f"Reman videos: {len(vid_list)}")
    lock = Lock()
